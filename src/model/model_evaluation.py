...
# This script handles the model training phase of a machine 
# learning pipeline. It loads the processed Bag of Words (BOW) 
# features, retrieves model parameters from a YAML configuration file, 
# trains a Gradient Boosting Classifier, and saves the trained model 
# for later use in evaluation or deployment.
...
# ---------------- Logging Configuration ----------------
# DEBUG logs help in development, while ERROR logs are stored in a file.
...
# File handler -> only ERROR logs saved
file_handler = logging.FileHandler('model_building_errors.log')
file_handler.setLevel('ERROR')
...
def load_data(file_path: str) -> pd.DataFrame:
    """Load processed Bag of Words (BOW) data from a CSV file."""
    try:
        df = pd.read_csv(file_path)
        logger.debug('Data loaded successfully from %s', file_path)
        return df
...
def main():
    ...
    Steps:
    1. Load model parameters from params.yaml.
    2. Load processed Bag of Words training & test data.
    3. Train a Gradient Boosting Classifier.
    4. Save the trained model to models/.
    5. Evaluate on test set and save metrics.
    """
    try:
        # Load hyperparameters
        params = load_params('params.yaml')['model_building']

        # Load BOW training data
